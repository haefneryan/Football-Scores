...
function loadData() {
  ...
  let api_token =
    "CHPO1UMSVjcgZA8oI6xmeLIxzAtVDXeiY6fypOF7npPkB7oqvjniWkg5Z2np";
  let seasonID = null;

  do {
    seasonID = 18369; //ID for current season
    if (document.getElementById("season").value == "2020/21") {
      seasonID = 17141;
    }
    if (document.getElementById("season").value == "2019/20") {
      seasonID = 16222;
    }
  } while (seasonID == null);
  let url =
    "https://soccer.sportmonks.com/api/v2.0/standings/season/" +
    seasonID +
    "?api_token=" +
    api_token;
  ...
  request.onload = function () {
    ...
    for (let j = 0; j < x; j++) {
      document.getElementById("scottishPremiershipTableHeader").innerHTML =
        "<div><div class='position'>Pos</div><div class='teamName'>Club</div><div class='tableData'>GP</div><div class='tableData'>W</div><div class='tableData'>D</div><div class='tableData'>L</div><div class='tableData'>Pts</div></div>";
      document.getElementById("scottishPremiershipTable").innerHTML +=
        "<div id=position_'" +
        j +
        "' class='tableRow'><div class='position'>" +
        table[j][0] +
        "</div><div class='teamName'>" +
        table[j][1] +
...
function loadData_2() {
  ...
  request.onload = function () {
    ...
    for (let i = 0; i < x; i++) {
      ...
      if (seasonID == 16020) {
        table[i][2] = JSON.stringify(
          response.data[0].standings.data[i].total.points
        );
      }
    }
    ...
    for (let j = 0; j < x; j++) {
      document.getElementById("superligaTableHeader").innerHTML =
        "<div><div class='position'>Pos</div><div class='teamName'>Club</div><div class='tableData'>GP</div><div class='tableData'>W</div><div class='tableData'>D</div><div class='tableData'>L</div><div class='tableData'>Pts</div></div>";
