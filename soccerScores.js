  ...
  request.onload = function () {
    ...
    for (let j = 0; j < x; j++) {
      if (matchUp[j][0] == "Premiership") {
        ...
        document.getElementById("scottishPremiershipName").innerHTML =
          "Scottish " + matchUp[j][0];
        document.getElementById("scottishPremiership").innerHTML +=
          "<br><div id=matchUp_'" +
          j +
          "' class='matchUp'><div class='home'><img src=" +
          matchUp[j][6] +
          " class='teamLogoHome'><div id='homeTeam_" +
          j +
          "' class='homeTeam'>" +
          matchUp[j][1] +
          "</div><div id='homeTeamScore_" +
          j +
          "'class='val score'>" +
          matchUp[j][2] +
          "</div></div><div class='val' id='dash'>-</div><div class='away'><div id='awayTeamScore_" +
          j +
          "' class='val score'>" +
          matchUp[j][3] +
          "</div><div id='awayTeam_" +
          j +
          "' class='awayTeam'>" +
          matchUp[j][4] +
          "</div><img src=" +
          matchUp[j][7] +
          " class='teamLogoAway'></div></div>";
    ...
    for (let j = 0; j < x; j++) {
      if (matchUp[j][0] == "Superliga") {
        ...
        document.getElementById("superligaName").innerHTML =
          "Danish " + matchUp[j][0];
    ...
    for (let i = 0; i < x; i++) {
      if (matchUp[i][2] > matchUp[i][3]) {
        document.getElementById("homeTeamScore_" + i).style.backgroundColor =
          "green";
        document.getElementById("awayTeamScore_" + i).style.backgroundColor =
          "red";
      }
      if (matchUp[i][2] < matchUp[i][3]) {
        document.getElementById("homeTeamScore_" + i).style.backgroundColor =
...
function submitchange() {
  ...
  if (document.getElementById("league").value == "scottishPremiership_select") {
    ...
    document.getElementById("scottishPremiership_scores").style.display =
      "block";
  }
